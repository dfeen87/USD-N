...
function usageAndExit() {
    console.error("Usage: usd-n-verify <log.jsonl> [--quiet|-q] [--strict] [--max-errors N]");
    process.exit(2);
}
async function readJsonl(path) {
    ...
    for await (const line of rl) {
        ...
        try {
            ...
            throw new Error(`JSONL_PARSE_ERROR at line ${lineNo}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return out;
}
...
function reviveBigints(e) {
    ...
    if (e?.type === "RESERVE_SNAPSHOT") {
        ...
        if (e.snapshot?.by_asset_usd) {
            for (const k of Object.keys(e.snapshot.by_asset_usd)) {
                ...
                if (typeof v === "string")
                    e.snapshot.by_asset_usd[k] = BigInt(v);
            }
        }
    ...
    if ((e?.type === "POLICY_ACTION" || e?.type === "POLICY_REJECTED") &&
